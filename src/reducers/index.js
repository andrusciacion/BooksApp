// import bookListReducer from './booksList';
...
const bookListReducer = (state = numberOfBooks, action) => {
  switch (action.type) {
    case ADD_BOOK:
      let newObj = [...state];
      newObj.push(action.book);
      return newObj;
    case EDIT_BOOK:
      let editedObj = [...state];
      editedObj.forEach((item, key) => {
        if (item.id === action.book.id) {
          editedObj.splice(key, 1);
          editedObj.push(action.book);
        }
      });
      return editedObj;
    case ADD_CART:
